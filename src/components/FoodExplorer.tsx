...
import { MealDetails } from '../utils/FetchMealDetails';
...
const nodeTypes = { startNode: StartNode, entityNode: EntityNode, optionNode: OptionNode };

const FoodExplorer = () => {
  const [nodes, setNodes] = useState<Node[]>(initialNodes);
  const [edges, setEdges] = useState<Edge[]>(initialEdges);
  const [mealDetails, setMealDetails] = useState<MealDetails | null>(null);
  const [mealLoading, setMealLoading] = useState<boolean>(true);
  ...
  const onNodeClick = async (event: React.MouseEvent, node: any) => {
    ...
    if (node.id === '1') {
      ...
      const newNodes = nodes.filter(n => n.type !== 'optionNode' && n.data.type !== 'meal' && n.data.type !== "ingredient" && n.data.type !== "tags");
      ...
      if (!existingMeals) {
        await FetchMeals(node.data.category).then((data) => {
          const newMeals = data.topMeals.map((meal, index) => {
            const mealNode = {
              id: 'meal-' + meal.idMeal,
              position: { x: node.position.x + 150, y: node.position.y + 140 - index * 100 },
      ...
      const newEdges: Edge[] = edges.filter(e => e.id !== 'me-1' && e.id !== 'oe-2' && e.id !== 'oe-3' && e.id !== 'oe-4' && !e.id.includes("ingredient") && !e.id.includes("tag"));
      const ingridientsNodesRemove = nodes.filter(n => n.data.type !== "ingredient" && n.data.type !== "tags")

      const mealEdge: Edge = {
        ...existingEdge, target: node.id
      };
      ...
      const optionNodes = [
        {
          ...
          data: { label: 'View Ingridients', id: node.id },
          type: 'optionNode',
          sourcePosition: 'right',
          targetPosition: 'left',
        },
      ...
      setNodes([...ingridientsNodesRemove, ...optionNodes])
      setEdges([...newEdges, ...optionEdges, mealEdge]);
    }
    else if (node.id === 'ingredients') {

      const newNodes = nodes.filter(n => n.data.type !== "tags" && n.data.type !== "ingredient")
      const newEdges = edges.filter(e => !e.id.includes("ingredient") && !e.id.includes("tag"));

      await FetchIngredients(node.data.id.split('-')[1]).then(data => {

        const ingredients = data.map((ing, index) => {
          const ingredientNode = {
            id: 'ing-' + index,
            position: { x: node.position.x + 170, y: node.position.y + 140 - index * 70 },
      ...
      await FetchTags(node.data.id.split('-')[1]).then(data => {
        if (data.length === 0) {
        ...
        const tags = data.map((tag, index) => {
          const tagNode = {
            id: 'tag-' + index,
            position: { x: node.position.x + 170, y: node.position.y + 70 - index * 70 },
      ...
      await FetchMealDetails(node.data.id.split('-')[1]).then((data) => {
        setMealDetails(data)
  ...
  return (
    ...
    <div className="">
      ...
      <ToastContainer />
      <Sidebar isOpen={isSidebarOpen} closeSidebar={closeSidebar} loading={mealLoading} details={mealDetails} />
    </div>
  );
};
